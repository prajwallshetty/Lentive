...
export default function MobileNav() {
  ...
  const isSearch = pathname === '/search' || pathname === '/listings';
      ...
      <div className="flex justify-around items-center h-14 max-w-lg mx-auto relative">
        ...
        <Link
          ...
          className={`flex flex-col items-center justify-center transition-all duration-300 active:scale-95 group relative w-14 ${
            isHome ? 'text-primary dark:text-[#34d399]' : 'text-muted-foreground hover:text-foreground'
          }`}
          aria-label="Home"
        >
          <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 ${
            ...
            <Home className={`h-5 w-5 transition-all duration-300 group-hover:-translate-y-0.5 ${
              isHome ? 'stroke-[2.5px] text-primary dark:text-[#34d399]' : 'stroke-[2px]'
            }`} />
          </div>
          <span className={`text-[8px] font-bold uppercase tracking-widest mt-1 transition-all duration-300 ${
            isHome ? 'opacity-100 scale-100 font-extrabold' : 'opacity-70 scale-95'
          }`}>
            Home
          </span>
          {/* Active glow dot */}
          <span className={`absolute -bottom-1.5 w-1 h-1 rounded-full bg-primary dark:bg-[#34d399] transition-all duration-300 ${
            isHome ? 'opacity-100 scale-100 shadow-[0_0_8px_var(--primary)]' : 'opacity-0 scale-50'
          }`} />
        </Link>
        ...
        <Link
          href="/search"
          className={`flex flex-col items-center justify-center transition-all duration-300 active:scale-95 group relative w-14 ${
            isSearch ? 'text-primary dark:text-[#34d399]' : 'text-muted-foreground hover:text-foreground'
          }`}
          aria-label="Search"
        >
          <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 ${
            ...
            <Search className={`h-5 w-5 transition-all duration-300 group-hover:scale-110 ${
              isSearch ? 'stroke-[2.5px] text-primary dark:text-[#34d399]' : 'stroke-[2px]'
            }`} />
          </div>
          <span className={`text-[8px] font-bold uppercase tracking-widest mt-1 transition-all duration-300 ${
            isSearch ? 'opacity-100 scale-100 font-extrabold' : 'opacity-70 scale-95'
          }`}>
            Search
          </span>
          {/* Active glow dot */}
          <span className={`absolute -bottom-1.5 w-1 h-1 rounded-full bg-primary dark:bg-[#34d399] transition-all duration-300 ${
            isSearch ? 'opacity-100 scale-100 shadow-[0_0_8px_var(--primary)]' : 'opacity-0 scale-50'
          }`} />
        </Link>
